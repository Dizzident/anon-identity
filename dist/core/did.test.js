...
describe('DIDService', () => {
    describe('createDIDKey', () => {
        ...
        it('should create consistent DIDs for the same public key', async () => {
            const keyPair = await crypto_1.CryptoService.generateKeyPair();
            const did1 = did_1.DIDService.createDIDKey(keyPair.publicKey);
            const did2 = did_1.DIDService.createDIDKey(keyPair.publicKey);
    ...
    describe('getPublicKeyFromDID', () => {
        it('should extract public key from did:key', async () => {
            const keyPair = await crypto_1.CryptoService.generateKeyPair();
            const did = did_1.DIDService.createDIDKey(keyPair.publicKey);
            const extractedPublicKey = did_1.DIDService.getPublicKeyFromDID(did.id);
            expect(extractedPublicKey).toEqual(keyPair.publicKey);
        });
        it('should throw error for invalid DID format', () => {
            expect(() => {
                did_1.DIDService.getPublicKeyFromDID('did:invalid:123');
    ...
    describe('createDIDDocument', () => {
        it('should create a valid DID document', async () => {
            const keyPair = await crypto_1.CryptoService.generateKeyPair();
            const did = did_1.DIDService.createDIDKey(keyPair.publicKey);
            const didDocument = await did_1.DIDService.createDIDDocument(did);
